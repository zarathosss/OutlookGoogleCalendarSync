...
namespace OutlookGoogleCalendarSync.Sync {
    public class SyncTimer : Timer {
        ...
        private void ogcsTimer_Tick(object sender, EventArgs e) {
            ...
            frm.NotificationTray.ShowBubbleInfo("Autosyncing calendars: " + Settings.Instance.ActiveCalendarProfile.SyncDirection.Name + "...");
        ...
        private int getResyncInterval() {
            int min = Settings.Instance.ActiveCalendarProfile.SyncInterval;
            if (Settings.Instance.ActiveCalendarProfile.SyncIntervalUnit == "Hours") {
                min *= 60;
            }
            return min;
        }
        ...
        public void SetNextSync(int? delayMins = null, Boolean fromNow = false) {
            ...
            if (Settings.Instance.ActiveCalendarProfile.SyncInterval != 0) {
                ...
                if (Settings.Instance.ActiveCalendarProfile.OutlookPush) Forms.Main.Instance.NextSyncVal += " + Push";
    ...
    public class PushSyncTimer : Timer {
        ...
        private PushSyncTimer() {
            ...
            Forms.Main.Instance.NextSyncVal = Settings.Instance.ActiveCalendarProfile.SyncInterval == 0 
                ? "Push Sync Active" 
        ...
        private void ogcsPushTimer_Tick(object sender, EventArgs e) {
            ...
            try {
                ...
                if (items.Count < this.lastRunItemCount || items.FindAll(x => x.LastModificationTime > this.lastRunTime).Count > 0) {
                    ...
                    Forms.Main.Instance.NotificationTray.ShowBubbleInfo("Autosyncing calendars: " + Settings.Instance.ActiveCalendarProfile.SyncDirection.Name + "...");
                    if (!Sync.Engine.Instance.SyncingNow) {
        ...
        public void Switch(Boolean enable) {
            if (enable && !ogcsTimer.Enabled) {
                ResetLastRun();
                ogcsTimer.Start();
                if (Settings.Instance.ActiveCalendarProfile.SyncInterval == 0) Forms.Main.Instance.NextSyncVal = "Push Sync Active";
